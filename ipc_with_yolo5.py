import sys # for input video checking
import os # for default input video checking
import subprocess # for the Inter-Process Communication
import numpy as np # for the tensor transformations
import torch # for YOLOv5
import cv2 # to show the results of the training
import warnings # to resolve warning problems
...
while True: # infinite loop
    ...
    # Returned tensor is bidimensional: [N, 6] -> (x1, y1, x2, y2, confidence, class)
    if predictions is not None and len(predictions) > 0: # if there's at least an object
        ...
        scales = np.array([scale_x, scale_y, scale_x, scale_y]) # to scale each coordinate 
        boxes = (boxes * scales).astype(int) # removes decimal figures from these new scaled pixel coordinates 

        confidences = predictions[:, 4].cpu().numpy() # confidence between 0.0 and 1.0
        coco_classes = predictions[:, 5].cpu().numpy().astype(int) # COCO class ID, although for example a tree class is missing (time for the 5070 Ti!!!)
    ...
    cv2.imshow("Progetto Aspromonte first attempt with YOLOv5", frame_resized)
